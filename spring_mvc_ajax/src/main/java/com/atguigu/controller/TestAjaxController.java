 ...
 *      要求：
 *          1）导入jackson的依赖
 *          2）在springMVC的配置文件中设置开启注解驱动（之前已经开启过了）
 *          3）在处理请求的控制器方法的形参的位置，直接设置json格式的请求参数要转换的java类型的形参
 *              接着使用@RequestBody注解标识即可
 * 3.@ResponseBody:
 *      作用：将所标识的控制器方法的返回值作为响应报文的响应体响应到浏览器
 * 4.使用@ResponseBody注解响应浏览器json格式的数据
 *      步骤：
 *      1）导入jackson的依赖
 *      2）在springMVC的配置文件中设置开启注解驱动（之前已经开启过了）
 *      3）将需要转换为json字符串的java对象直接作为控制器方法的返回值，使用@ResponseBody注解
 *          标识控制器方法，就可以将java对象直接转换为json字符串，并响应到浏览器
 *
 * 常用的java对象转换为json的结果
 * 实体类-->json对象
 * map --> json对象
 ...
 */
@Controller
//@RestController  相当于 @Controller(类上) + @ResponseBody（类中每个方法上）
